...
const lawCategories = {
  ...
  "施行令・規則": [
    ...
    { id: "conveyor", name: "コンベヤ安全規則" },
    { id: "yukiyouzai", name: "有機溶剤中毒予防規則" },
    { id: "namari", name: "鉛中毒予防規則" },
    { id: "yonalkyl", name: "四アルキル鉛中毒予防規則" },
    { id: "tokutei", name: "特定化学物質障害予防規則" },
    { id: "koukiatsu", name: "高気圧作業安全衛生規則" },
    { id: "denri", name: "電離放射線障害防止規則" },
    { id: "sanso", name: "酸素欠乏症等防止規則" },
    { id: "jimusho", name: "事務所衛生基準規則" },
    { id: "funjin", name: "粉じん障害防止規則" },
    { id: "touroku", name: "登録及び指定に関する省令" },
    { id: "kikai", name: "機械等検査規則" },
    { id: "consultant", name: "労働安全衛生コンサルタント規則" },
    { id: "sekimen", name: "石綿障害予防規則" },
    { id: "josen", name: "除染等電離放射線障害防止規則" }
  ],
  "その他関連法": [
    { id: "rousai", name: "労働災害補償保険法" },
    { id: "kankyousokutei", name: "作業環境測定法" },
    { id: "koyouhoken", name: "雇用保険法" },
    { id: "haken", name: "労働者派遣法" }
  ]
};
...
function SafetyLawTracker() {
  ...
  const [revisions, setRevisions] = useState(sampleRevisions);
  const [loading, setLoading] = useState(true);
  const [dataSource, setDataSource] = useState("sample");

  // JSONファイルからデータを読み込む
  useEffect(() => {
    fetch('/data/revisions.json')
      .then(response => {
        if (!response.ok) {
      ...
      .then(data => {
        console.log('✅ 実データを読み込みました:', data.length, '件');
        setRevisions(data);
        setDataSource("live");
        setLoading(false);
      })
      .catch(error => {
        console.log('ℹ️ 実データが見つからないため、サンプルデータを使用します');
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          <div className="text-xl font-bold text-gray-800 mb-2">データを読み込んでいます...</div>
          <div className="text-sm text-gray-600">しばらくお待ちください</div>
        </div>
      </div>
    );
  }
              ...
              <div className="bg-green-600 px-4 py-2 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm">
                    <div className="font-bold">自動更新データ</div>
                    <div className="text-green-100 text-xs">GitHub Actions</div>
                  </div>
                </div>
              </div>
            )}
            {dataSource === "sample" && (
              <div className="bg-yellow-600 px-4 py-2 rounded-lg">
                <div className="flex items-center gap-2">
                  <AlertCircle size={20} />
                  <div className="text-sm">
                    <div className="font-bold">サンプルデータ</div>
                    <div className="text-yellow-100 text-xs">デモ表示中</div>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            {revisionStages.map((stage, index) => {
              ...
              const count = revisions.filter(r => r.stage === stage.id).length;
              return (
